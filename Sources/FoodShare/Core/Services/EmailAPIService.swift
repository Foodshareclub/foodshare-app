...
actor EmailAPIService {
    ...
    func sendTemplateEmail(to: String, template: String, data: [String: Any]) async throws {
        let _: EmptyResponse = try await client.post("api-v1-email/send-template", body: [
            "to": to,
            "template": template,
            "data": data
        ])
